...
private:
  ...
  enum class MissionState { WAITING_TO_START, START_REQUESTED, INSPECTION, INSPECTION_COMPLETE, NAVIGATION_CONTINUE };
  ...
  void read_parameters()
  {
    ...
    if (start_delay_s_ < 0) throw std::runtime_error("start_delay_s must be >= 0");
    if (inspection_duration_s_ <= 0) throw std::runtime_error("inspection_duration_s must be > 0");
  }

  void setup_publisher()
  {
    phase_publisher_ = create_publisher<std_msgs::msg::String>("/mission/phase", rclcpp::QoS(rclcpp::KeepLast(10)).reliable());
  }
  ...
  void setup_timer()
  {
    timer_ = create_wall_timer(std::chrono::milliseconds(publish_period_ms_), std::bind(&MissionManagerNode::timer_callback, this));
  }
  ...
  void timer_callback()
  {
    ...
    if (state_ == MissionState::INSPECTION) {
      const double inspection_elapsed_s = (now() - inspection_start_time_).seconds();
    ...
    if (state_ == MissionState::INSPECTION_COMPLETE) {
      const double complete_elapsed_s = (now() - inspection_complete_time_).seconds();
  ...
  void request_mission_active(bool active)
  {
    if (request_pending_) return;
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "Management service not ready");
      return;
    }
    ...
    mission_client_->async_send_request(request, [this, active](std::shared_future<SetBool::Response::SharedPtr> future) {
      request_pending_ = false;
      const auto response = future.get();
        RCLCPP_WARN(get_logger(), "Mission request rejected: %s", response->message.c_str());
        if (active) state_ = MissionState::WAITING_TO_START;
        return;
      }
      RCLCPP_INFO(get_logger(), "Mission request accepted");
      if (active) {
  ...
  void publish_phase()
  {
    ...
    switch (state_) {
      case MissionState::WAITING_TO_START: case MissionState::START_REQUESTED: phase.data = "IDLE"; break;
      case MissionState::INSPECTION: phase.data = "INSPECTION"; break;
      case MissionState::INSPECTION_COMPLETE: phase.data = "INSPECTION_COMPLETE"; break;
      case MissionState::NAVIGATION_CONTINUE: phase.data = "NAVIGATION_CONTINUE"; break;
    }
    phase_publisher_->publish(phase);
  }

  int publish_period_ms_, start_delay_s_, inspection_duration_s_;
  bool request_pending_{false};
  MissionState state_{MissionState::WAITING_TO_START};
  rclcpp::Time node_start_time_{0, 0, RCL_ROS_TIME}, inspection_start_time_{0, 0, RCL_ROS_TIME}, inspection_complete_time_{0, 0, RCL_ROS_TIME};
